...
test('v2 : les données par défaut ont templates et plans, et une v1 est migrée', () => {
  ...
  assert.equal(store2.getData().schemaVersion, 3);
  assert.equal(store2.getData().sessions.length, 1);
...
test('v3 : une sauvegarde v2 (exerciseIds) est migrée vers items avec 3×10 par défaut', () => {
  const store = createStore(storageFactice());
  const v2 = JSON.parse(store.export());
  v2.schemaVersion = 2;
  v2.templates = [{ id: 't1', name: 'Haut', exerciseIds: ['exo-1', 'exo-2'] }];
  v2.plans = [{ id: 'p1', date: '2026-07-05', templateId: 't1', name: 'Haut', exerciseIds: ['exo-1'] }];
  store.import(JSON.stringify(v2));
  const d = store.getData();
  assert.equal(d.schemaVersion, 3);
  assert.deepEqual(d.templates[0].items, [
    { exerciseId: 'exo-1', sets: 3, reps: 10 },
    { exerciseId: 'exo-2', sets: 3, reps: 10 },
  ]);
  assert.equal(d.plans[0].items.length, 1);
});
...
test('v3 : CRUD des modèles de séance avec objectifs séries × reps', () => {
  ...
  assert.equal(store.getData().templates[0].name, 'Haut V2');
  assert.deepEqual(store.getData().templates[0].items, [{ exerciseId: 'exo-1', sets: 5, reps: 8 }]);
  store.deleteTemplate(t.id);
  assert.equal(store.getData().templates.length, 0);
});

test('setExerciseRest : mémorise le repos par exercice, null le retire', () => {
  const store = createStore(storageFactice());
  store.setExerciseRest('exo-1', 120);
  assert.equal(store.getData().exercises.find((e) => e.id === 'exo-1').rest, 120);
  assert.equal(store.getData().exercises.find((e) => e.id === 'exo-2').rest, undefined);
  store.setExerciseRest('exo-1', null);
  assert.equal('rest' in store.getData().exercises.find((e) => e.id === 'exo-1'), false);
  // Exercice inconnu : sans effet, ne plante pas
  store.setExerciseRest('exo-inconnu', 60);
});
...
test('v3 : poser un modèle sur un jour crée une copie indépendante (items compris)', () => {
  ...
  assert.equal(store.getData().templates[0].items.length, 2);
  assert.equal(store.getData().templates[0].items[0].sets, 4);
  assert.equal(store.getData().plans[0].name, 'Jambes light');
  assert.equal(store.getData().plans[0].items[0].sets, 2);
